...
app.post("/api/connect", async (request, response) => {
  ...
  if (!/^\d+$/.test(userId.trim())) {
    return response.status(400).json({ error: "User ID Roblox harus berupa angka." });
  }
  try {
    const profileResponse = await fetch(`https://users.roblox.com/v1/users/${userId.trim()}`);
    if (!profileResponse.ok) return response.status(404).json({ error: "User ID Roblox tidak ditemukan." });
    const profile = await profileResponse.json();
    const thumbnailResponse = await fetch(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${userId.trim()}&size=150x150&format=Png&isCircular=true`);
    ...
    return response.status(502).json({ error: "Layanan Roblox sedang tidak dapat dihubungi." });
  }
});
